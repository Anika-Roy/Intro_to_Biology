        ...
        if num_1<num_2:
            ...
            for i in range(1, (rows+1)):
                ...
                for j in range(1, (cols+1)):
                    list = [[values[i-1][j-1]+weights[i-1][j-1], length[i-1][j-1]], [values[i-1][j], length[i-1][j]],[values[i][j-1],length[i][j-1]]]
                    list.sort()
                    list.reverse()
            ...
            print((length[rows][cols]-values[rows][cols])/length[rows][cols],end=',')

            #print((length-values[rows][cols])/length,end=',')
        else:
            print(0,end=',')
